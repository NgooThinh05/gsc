...
export default function WarehouseOrderDetailPage() {
  ...
  useEffect(() => {
    loadPageData().catch((error) => setAlert({ msg: error.message, type: 'error' }));
  }, []);
  ...
  async function approveOrder() {
    ...
    try {
      ...
      await loadPageData();
      setAlert({ msg: 'Đã xác nhận số lượng giao, cập nhật kho và chuyển đơn sang phần giao hàng', type: 'success' });
    } catch (error) {
      setAlert({ msg: error.message, type: 'error' });
    }
  }
