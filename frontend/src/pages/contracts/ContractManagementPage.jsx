...
export default function ContractManagementPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      await loadData();
      setAlert({ msg: 'Tạo hợp đồng thành công', type: 'success' });
    } catch (error) {
  ...
  async function handleExtendContract(contractId) {
    ...
    if (!NgayHetHan) {
      setAlert({ msg: 'Vui lòng chọn ngày hết hạn mới', type: 'warning' });
      return;
    }
    ...
    try {
      ...
      await loadData();
      setAlert({ msg: 'Gia hạn hợp đồng thành công', type: 'success' });
    } catch (error) {
      setAlert({ msg: error.message, type: 'error' });
    }
  }
